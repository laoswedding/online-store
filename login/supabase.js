...
(async function restoreSession() {
  ...
  const { data: profile } = await db
    ...
    .single();
  session = { ...data.session, token: data.session.access_token, role: profile?.role || 'viewer' };
  enterApp();
})();
...
const itemFromDb = (r) => ({
  id: r.id, name: r.name, price: r.price, inStock: r.in_stock,
  description: r.description, category: r.category, imgSrc: r.img_src,
  learnMore: r.learn_more, updatedBy: r.updated_by, updatedAt: r.updated_at,
});

const ITEM_COLUMNS = {
  name: 'name', price: 'price', inStock: 'in_stock', description: 'description',
  category: 'category', imgSrc: 'img_src', learnMore: 'learn_more',
};
function itemToDb(obj) {
  const row = {};
  for (const [key, col] of Object.entries(ITEM_COLUMNS)) if (key in obj) row[col] = obj[key];
  return row;
}
...
const orderFromDb = (r) => {
  ...
  return {
    ...
    shippingCompany: r.shipping_company, branch: r.branch, province: r.province,
    city: r.city, village: r.village, photoUrl: r.photo_url,
    timestamp: r.created_at, orderId: r.order_id, orderTotal: r.order_total,
    orderTotalItems: r.order_total_items, orderStatus: r.order_status,
    tracking: r.tracking, hasDiscountCode: r.has_discount_code, discountCode: r.discount_code,
    ...
    orderItemsImgSrc: lines.map((l) => (l.items && l.items.img_src) || '').join(', '), // CHECK format
...
async function withSlipUrls(orders) {
  const paths = orders.map((o) => o.photoUrl).filter((p) => p && !/^https?:/i.test(p));
  if (!paths.length) return orders;
  const { data } = await db.storage.from('payment-slips').createSignedUrls(paths, 3600);
  const urls = Object.fromEntries((data || []).filter((d) => d.signedUrl).map((d) => [d.path, d.signedUrl]));
  return orders.map((o) => (urls[o.photoUrl] ? { ...o, photoUrl: urls[o.photoUrl] } : o));
}

const HANDLERS = {
  listItems: async () => {
    await requireSession();
    return (await run(db.from('items').select('*').order('name'))).map(itemFromDb);
  },
  createItem: async (_token, item) => {
    await requireSession();
    const row = itemToDb(item);
    if (item.id) row.id = item.id;   // otherwise the database generates one
    return itemFromDb(await run(db.from('items').insert(row).select().single()));
  },
  updateItem: async (_token, id, changes) => {
    await requireSession();
    return itemFromDb(await run(
      db.from('items').update(itemToDb(changes)).eq('id', id).select().single()));
  },
  ...
  listOrders: async () => {
    await requireSession();
    const rows = await run(db.from('orders')
  ...
  cancelOrder: (orderId) => run(db.rpc('cancel_order', { p_order_id: orderId })),
